...
public class Mk10SignUp extends Mk10Initializer implements MouseListener, KeyListener {
    ...
    ///判断注册时用户名是否被占用
    public static boolean checkUsernameUsed(ArrayList<Mk10User> library, String username) {
        boolean flag = false;
    ...
    void signup() {
        if (checkUsername(username) && !checkUsernameUsed(library, username) && checkPassword(password) && password.equals(passwordAgain) && checkPhoneNumber(phoneNumber) && !checkPhoneNumberUsed(library, phoneNumber) && inputCode.equals(codeTemp)) {
            library.add(new Mk10User(username, password, phoneNumber));
            setVisible(false);
            new Mk10Login();
        } else {
