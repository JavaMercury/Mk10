...
public abstract class Mk10Initializer extends JFrame {
    //数字字符-48为实际数字，十六进制大写字母-55

    //创建集合，存储用户信息
    static ArrayList<Mk10User> library = new ArrayList<>();

    //测试用账号
    static {
        library.add(new Mk10User("qwerqwer", "12341234Aa", "12312341234"));
    }

    String version = "水银第十代 0.10.16.20230203";
    String username;
    String password;
    ...
    ///判断重设的手机号码是否被占用
    public static boolean checkPhoneNumberUsed(ArrayList<Mk10User> library, String phoneNumber) {
        boolean flag = false;
        for (Mk10User user : library) {
    ...
    ///判断手机号码是否重复
    public static boolean checkSamePhoneNumber(ArrayList<Mk10User> library, String username, String phoneNumber) {
        Mk10User user = library.get(getUserIndex(library, username));
    ...
    ///获取用户名对应的索引
    public static int getUserIndex(ArrayList<Mk10User> library, String username) {
        int index;
        for (index = 0; index < library.size(); index++) {
            Mk10User user = library.get(index);
    ...
    ///判断密码是否重复
    public static boolean checkSamePassword(ArrayList<Mk10User> library, String username, String password) {
    ...
    ///判断输入新密码的用户是否存在
    public static boolean checkUserExist(ArrayList<Mk10User> library, String username) {
        for (Mk10User user : library) {
