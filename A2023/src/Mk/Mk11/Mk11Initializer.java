...
public abstract class Mk11Initializer extends JFrame {
    //数字字符-48为实际数字，十六进制大写字母-55

    //创建集合，存储用户信息
    static ArrayList<Mk11User> library = new ArrayList<>();

    //测试用账号
    static {
        library.add(new Mk11User("qwerqwer", "12341234Aa", "12312341234"));
    }

    String version = "水银第11代 0.11.00.20230209";
    String username;
    String password;
    ...
    ///判断重设的手机号码是否被占用
    public static boolean checkPhoneNumberUsed(ArrayList<Mk11User> library, String phoneNumber) {
        boolean flag = false;
        for (Mk11User user : library) {
    ...
    ///判断手机号码是否重复
    public static boolean checkSamePhoneNumber(ArrayList<Mk11User> library, String username, String phoneNumber) {
        Mk11User user = library.get(getUserIndex(library, username));
    ...
    ///获取用户名对应的索引
    public static int getUserIndex(ArrayList<Mk11User> library, String username) {
        int index;
        for (index = 0; index < library.size(); index++) {
            Mk11User user = library.get(index);
    ...
    ///判断密码是否重复
    public static boolean checkSamePassword(ArrayList<Mk11User> library, String username, String password) {
    ...
    ///判断输入新密码的用户是否存在
    public static boolean checkUserExist(ArrayList<Mk11User> library, String username) {
        for (Mk11User user : library) {
