...
public class Mk11SignUp extends Mk11Initializer implements MouseListener, KeyListener {
    ...
    ///判断注册时用户名是否被占用
    public static boolean checkUsernameUsed(ArrayList<Mk11User> library, String username) {
        boolean flag = false;
    ...
    void signup() {
        if (checkUsername(username) && !checkUsernameUsed(library, username) && checkPassword(password) && password.equals(passwordAgain) && checkPhoneNumber(phoneNumber) && !checkPhoneNumberUsed(library, phoneNumber) && inputCode.equals(codeTemp)) {
            library.add(new Mk11User(username, password, phoneNumber));
            setVisible(false);
            new Mk11Login();
        } else {
